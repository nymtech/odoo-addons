    ...
    def synchronize_revolut_accounts(self):
        ...
        for account in bank_accounts:
            ...
            for revolut_account in jwt_to_revolut_infos[revolut_jwt_id]:
                ...
                revolut_account_details = revolut_jwt_id.ask_revolut_api(
                    "/accounts/%s/bank-details" % revolut_account["id"]
                )
                if revolut_account_details:
                    ...
                    if valid_bank_account:
                        bank_accounts.filtered(
                            lambda a: a.sanitized_acc_number == valid_bank_account["iban"]
                            and a.currency_id.name == revolut_account.get("currency")
                        ).write(
                            {
