...
class JsonWebToken(models.Model):
    ...
    def _compute_is_revolut(self):
        for token in self:
            token.is_revolut = token.payload_line_ids.filtered(
                lambda l: l.key == "aud" and l.value == "https://revolut.com"
            ).exists()

    @api.depends("revolut_access_token_expiration")
    def _compute_revolut_token_expired(self):
        for token in self:
            token.revolut_token_is_expired = (
                token.revolut_access_token_expiration and token.revolut_access_token_expiration < fields.Datetime.now()
            )
    ...
    def _check_endpoint_whitelist(self, endpoint):
        ...
        if not any(re.match(r, endpoint) for r in regex_str_list):
            raise UserError(
                _("Revolut API endpoint '%s' is not whitelisted\nPlease check '%s' settings", endpoint, self.name)
            )
    ...
    def _update_revolut_token_infos(self, res, *args):
        ...
        if "expires_in" in args:
            infos["revolut_access_token_expiration"] = fields.Datetime.now() + relativedelta(
    ...
    def action_reset_authorized_endpoints(self):
        self.ensure_one()
        self.authorized_endpoint_ids = [(5, 0, 0)] + [
            (0, 0, {"name": endpoint}) for endpoint in DEFAULT_AUTHORIZED_ENDPOINTS
        ]
        return {}
